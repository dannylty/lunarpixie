...
class OpenAICompatProvider(LLMProvider):
    ...
    def _extract_timings(
        ...
        and ``timings.predicted_per_second`` on the final response/chunk by
        default. When speculative decoding is active it also reports ``draft_n``
        ...
        if response_map is not None:
            ...
            if not choices:
                ...
                if content is not None:
                    prefill_tps, generation_tps, draft_ar = self._extract_timings(
                        response_map
                    )
                    return LLMResponse(
                        content=content,
    ...
    def _parse_chunks(cls, chunks: list[Any]) -> LLMResponse:
        ...
        timings: tuple[float | None, float | None, float | None] = (
            None,
            None,
            None,
        )
        ...
        for chunk in chunks:
            ...
            if chunk_map is not None:
                ...
                if not choices:
                    ...
                    chunk_timings = cls._extract_timings(chunk_map)
                    if any(value is not None for value in chunk_timings):
            ...
            if not chunk.choices:
                ...
                chunk_timings = cls._extract_timings(chunk)
                if any(value is not None for value in chunk_timings):
                    timings = chunk_timings
                continue
        ...
        prefill_tps, generation_tps, draft_ar = timings
        return LLMResponse(
            content=content,
            tool_calls=tool_calls,
            finish_reason=finish_reason,
